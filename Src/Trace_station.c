...
{
    Move_Robot(DEFAULT_SPEED-1, -DEFAULT_SPEED+3);
}
...
static int Select_By_Long_Direction(IR_DIRECTION* direction, int reset)
{
    ...
    #endif
    if(IS_DETECTED_SHORT_ALL_DIRECTION(currentdirection) || IS_DETECTED_SHORT_CENTER_DIRECTION(currentdirection))
    {     
        //CenterShortCount = LeftShortCount = RightShortCount = 0;
        return FORWARD;
    }
    ...
    else if(IS_DETECTED_ALL_DIRECTION(currentdirection) || IS_DETECTED_CENTER_DIRECTION(currentdirection))
    {     
        //CenterShortCount = LeftShortCount = RightShortCount = 0;
        return FORWARD;
    }
...
static int Action_TraceLongIr(IR_DIRECTION* direction, int* currentmotion, int reset)
{
    ...
    if(StopCount >= STOP_THRES)
    {
        ...
        if(MissingDirection >= NUM_OF_ACTION)
        {
            #if 0
            Stop_Robot();
            #endif
        ...
        else if(MissingDirection == CHANGE_DIRECTION)
        {
            FlagMissForward++;
        }
		StopCount = 0;
...
static int Action_FindStation_Ver2(IR_DIRECTION* direction, int mode, int reset)
{
    static int FindLevel, FindResult;
    static int FindCountLongCenter, FindCountLongLeft, FindCountLongRight, FindCountShortCenter, FindCountShortLeft, FindCountShortRight;
    static int FindTime = FIND_TIME;
    ...
    if(FindTime > 0)
    {
        ...
        {            
            if(mode == FIND_LONG)
            {
                if(IS_DETECTED_LEFT_CENTER_DIRECTION(currentdirection))
                {
                    FindCountLongLeft++;
                    if(FindCountLongLeft >= IR_COUNT(2))
                    {
                        FindResult = FIND_LEFT_CENTER;
                    }
                }
                else
                {
        ...
        {      
            if(mode == FIND_LONG)
            {
                if(IS_DETECTED_RIGHT_CENTER_DIRECTION(currentdirection))
                {   
                    FindCountLongRight++;
                    if(FindCountLongRight >= IR_COUNT(2))
                    {
                        FindResult = FIND_RIGHT_CENTER;
                    }
...
static int Action_SetStartDirection(int* startdirection, int reset)
{
    ...
    if(!first)
    {
        ...
        switch(*startdirection)
        {
            case FIND_LEFT :
            {
                CountSetDirection = SET_DIRECTION_TIME * 1.2;
                break;
            }
            case FIND_RIGHT :
            {
                CountSetDirection = SET_DIRECTION_TIME * 1.1;
                break;
            }

            case FIND_LEFT_CENTER :
            {
                CountSetDirection = SET_DIRECTION_TIME * 1.1;
                break;
            }
            case FIND_RIGHT_CENTER :
            {
                CountSetDirection = SET_DIRECTION_TIME * 1.1;
                break;
            }
    ...
    {
        if(*startdirection == FIND_SHORT_LEFT || *startdirection == FIND_LEFT || *startdirection == FIND_LEFT_CENTER)
        {
            Turn_Left();
        }
        else if(*startdirection == FIND_SHORT_RIGHT || *startdirection == FIND_RIGHT || *startdirection == FIND_RIGHT_CENTER)
        {
            Turn_Right();
        }
...
static int Action_TracePhase1_Ver2(IR_DIRECTION* direction, int* startdirection, int reset)
{
    int result = 0;
    if(reset == 1)
    {
    ...
    result = Action_FindStation_Ver2(direction, FIND_LONG, 0);
    *startdirection = debug.detectlev = result;
    ...
    else if(result == FIND_LEFT_CENTER || result == FIND_RIGHT_CENTER || result == FIND_LEFT || result == FIND_RIGHT)
    {
        return MODE_SET_LONG_DIRECTION;
    }
    else if(result == REPEAT)
...
static int Action_TracePhase1point5_Ver2(IR_DIRECTION* direction, int* startdirection, int reset)
{
    int result = 0;
    if(reset == 1)
    {
...
{    
    static int MoveCount = MOVE_TIME*1.5;
    if(reset == 1)
    {
        MoveCount = MOVE_TIME*1.5;
        return 255;
    }
...
static int Action_TracePhase1point8_Ver2(int* startdirection, int reset)
{    
...
static int Action_TracePhase2(IR_DIRECTION* direction, int* startdirection, int reset)
{
    ...
    if(reset == 1)
    {
        result = Action_TraceLongIr(direction, &currentmotion, reset);
        return 255;
    }
...
{   
    ...
    if(sysdata.tof_org_rang_left <= 500 || sysdata.tof_org_rang_right <= 500)
    {
        Turn_Left();
    }
    #endif
    #ifdef MAIN
    killTimer(Timer_TOF_10ms);
    //static int TraceStationMode = MODE_FIND_SHORT_IR;
    static int TraceStationMode = MODE_FIND_LONG_IR;//Ver2
    static int StartDirection;
    int chargepg = 0;
    if(Get_ChargeMode() != 1)
    {
        TraceStationMode = MODE_FIND_LONG_IR;
        StartDirection = 0;
        return;
    }
    ...
    switch(TraceStationMode)
    {
        ...
        {            
            ...
            TraceStationMode = Action_TracePhase1_Ver2(&current_ir, &StartDirection, 0);//Ver2
            break;
        }
        case MODE_SET_LONG_DIRECTION :
        {
            //TraceStationMode = Action_TracePhase1point5(&current_ir, &StartDirection, 0);
            TraceStationMode = Action_TracePhase1point5_Ver2(&current_ir, &StartDirection, 0);//Ver2
            break;
        }
        case MODE_GOTO_CENTERLINE :
        {
            //TraceStationMode = Action_TracePhase1point8(0);
            TraceStationMode = Action_TracePhase1point8_Ver2(&StartDirection, 0);//Ver2 
            break;
        }
...
void Clear_InnerStaticVar()
{
    ...
    Action_FindStation_Ver2(0, 0, 1);
    Action_SetStartDirection(0, 1);
    Action_TracePhase0point8(1);
    Action_TracePhase1point8_Ver2(0, 1);
