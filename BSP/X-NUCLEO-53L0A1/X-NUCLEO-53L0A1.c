...
/*    GPIO_InitStruct.Pin = GPIO_PIN_8 | GPIO_PIN_9;
    ...
    XNUCLEO53L0A1_hi2c.Init.Timing = 0x00300F38; 
...
int XNUCLEO53L0A1_ResetId(int DevNo, int state) {
    int status;
    switch( DevNo ){
    case XNUCLEO53L0A1_DEV_CENTER : //FIRO에서는 FRONT
    case 'c' :
        if( state ) FRONT_SHUT_TOF_ON;
        else		FRONT_SHUT_TOF_OFF;
        break;
    case XNUCLEO53L0A1_DEV_LEFT :	//FIRO에서는 REAR
    case 'l' :
        if( state ) FRONTL_SHUT_TOF_ON;
        else		FRONTL_SHUT_TOF_OFF;
        break;
    case 'r' :
    case XNUCLEO53L0A1_DEV_RIGHT :
        if( state ) FRONTR_SHUT_TOF_ON;
        else		FRONTR_SHUT_TOF_OFF;
        break;
	case XNUCLEO53L0A1_DEV_DLEFT :
        if( state ) DOWNL_SHUT_TOF_ON;
        else		DOWNL_SHUT_TOF_OFF;
		break;
	case XNUCLEO53L0A1_DEV_DRIGHT :
        if( state ) DOWNR_SHUT_TOF_ON;
        else		DOWNR_SHUT_TOF_OFF;
		break;
	default:
        XNUCLEO53L0A1_ErrLog("Invalid DevNo %d",DevNo);
        status = -1;
